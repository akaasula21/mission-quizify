...
if __name__ == "__main__":
    ...
    if 'question_bank' not in st.session_state or len(st.session_state['question_bank']) == 0:
        st.session_state['question_bank'] = []  # Step 1: Initialize the question bank list in st.session_state
        ...
        with screen.container():
            ...
            with st.form("Load Data to Chroma"):
                ...
                topic_input = st.text_input("Topic for Generative Quiz",
                                            placeholder="Enter the topic of the document")  # Step 2: Set topic input and number of questions
                ...
                if submitted:
                    ...
                    st.session_state[
                        'question_bank'] = question_bank  # Step 4: Initialize the question bank list in st.session_state
                    st.session_state[
                        "display_quiz"] = True  # Step 5: Set a display_quiz flag in st.session_state to True
    ...
    if st.session_state.get("display_quiz", False):
        ...
        with st.container():
            ...
            with st.form("MCQ"):
                index_question = quiz_manager.get_question_at_index(st.session_state[
                                                                        "question_index"])  # Step 7: Set index_question using the Quiz Manager method get_question_at_index passing the st.session_state["question_index"]
